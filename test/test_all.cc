...
{
    kmean::point<float> a(3.56, 7.78, -9.98);
    kmean::point<float> b(3.56, 7.78, -9.98);
    kmean::point<float> c(-3.56, -7.78, -9.98);
    kmean::point<float> d(-3.56, -7.78, -9.98000000000000000001);
    BOOST_CHECK(a == b);
    BOOST_CHECK(!(a == c));
    BOOST_CHECK(c == d);
}
...
BOOST_AUTO_TEST_CASE(arithmetic)
{
    ...
    float eps = 0.0001;
    kmean::point<float> a(3.56, 7.78, -9.98);
    kmean::point<float> b(5.56, -7.078, 19.8);
    kmean::point<float> c(-3.56, -7.78, 9.98);
    kmean::point<float> sum = a + c;
...
BOOST_AUTO_TEST_CASE(dist) {
    float eps = 0.0001;
    kmean::point<float> p1(1.4, -2.6, 3.78), p2(-5.8, 56.7, -9.1);
    float dist = kmean::dist_euclid<float>(p1, p2);
    BOOST_CHECK_CLOSE(dist, 3734.2244, eps);

    kmean::point<float> p3(1.4, -2.6, 3.78);
    dist = kmean::dist_euclid<float>(p1, p3);
    BOOST_CHECK_CLOSE(dist, 0.0, eps);
}

BOOST_AUTO_TEST_CASE(closest) {
    kmean::point<float> p1(1, 2, 3);
    kmean::point<float> p2(11, 21, 31);
    std::vector<kmean::point<float> > pv;
    for (int i = 0; i < 4; i++) {
        kmean::point<float> tmp(i+10, i+20,i+30);
        pv.push_back(tmp);
    }
    int closest = kmean::find_closest_point<float>(p1, pv);
    BOOST_CHECK_EQUAL(closest, 0);
    closest = kmean::find_closest_point<float>(p2, pv);
